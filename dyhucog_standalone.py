# AI-Powered Dynamic Hybrid Recommender Systems for Movie Discovery: A Graph-Based Cooperative-Game (DyHuCoG) Framework

# Enhanced version with plots, additional metrics, baselines, diversity, cold-start evaluation
...
decay = 1e-4
batch_size = 256
epochs = 200  # Reduced for faster training, increase for better results
ks = [5, 10, 20]
dae_hidden = 128
fs_hidden = 128
fs_epochs = 50
...
if not os.path.exists(data_path):
    print("Please download MovieLens-100k and put in ml-100k/")
    exit()
...
class RecommenderDataset:
    ...
    def load_ratings(self):
        self.ratings = pd.read_csv(self.path + 'u.data', sep='\t', names=['user', 'item', 'rating', 'timestamp'], header=None)
        ...
        # Train test split
        self.train_ratings, self.test_ratings = train_test_split(self.ratings, test_size=0.2, random_state=seed)
    ...
    def load_items(self):
        ...
        self.items = pd.read_csv(self.path + 'u.item', sep='|', names=['item', 'title', 'release_date', 'video_release_date', 'imdb_url'] + genre_cols, encoding='latin-1', header=None)
    ...
    def build_graph(self):
        # Nodes: users 1 to n_users, items n_users+1 to n_users+n_items, genres n_users+n_items+1 to ...
        ...
        self.adj = torch.sparse.FloatTensor(indices, values, (self.total_nodes + 1, self.total_nodes + 1)).to(device)
        # Norm adj
    ...
    def update_norm_adj(self):
        ...
        d_indices = torch.stack((torch.arange(self.total_nodes + 1, device=device), torch.arange(self.total_nodes + 1, device=device)))
        d_values = d_inv_sqrt
        d_mat = torch.sparse.FloatTensor(d_indices, d_values, (self.total_nodes + 1, self.total_nodes + 1)).to(device)
...
def train_dae(dataset):
    ...
    for epoch in range(dae_epochs):
        ...
        print(f'DAE Epoch {epoch}, Loss {avg_loss}')
    plt.plot(losses)
...
def train_fs(dataset, dae):
    ...
    for epoch in range(fs_epochs):
        ...
        losses.append(avg_loss)
        print(f'FS Epoch {epoch}, Loss {avg_loss}')
    plt.plot(losses)
    plt.title('FastSHAP Training Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('fs_loss.png')
    plt.close()
    return fs
    ...
    with torch.no_grad():
        for u in range(1, dataset.n_users + 1):
            ...
            for j in observed_indices:
                i = j.item()
                w = phi[j].item()
                weights[(u, i)] = max(w, 0) + 1  # add 1 to avoid zero or negative
    return weights
...
def update_graph(dataset, weights):
    ...
    dataset.adj = torch.sparse.FloatTensor(indices, values, (dataset.total_nodes + 1, dataset.total_nodes + 1)).to(device)
...
def compute_diversity(rec_items, item_genres, n_genres):
    genres = set()
    for i in rec_items:
        genres.update(item_genres[i])
    return len(genres) / n_genres if n_genres > 0 else 0
...
def evaluate(model, dataset, users_list, ks, prefix=''):
    ...
    with torch.no_grad():
        for u in users_list:
            ...
            coverage_set.update(rec_items[:max(ks)])
            div = compute_diversity(rec_items[:max(ks)], dataset.item_genres, len(dataset.genre_cols))
            diversity += div
            for k in ks:
    ...
    coverage = len(coverage_set) / dataset.n_items
    diversity /= num_users
    ...
    return metrics, coverage, diversity
...
def train(model, dataset, model_name):
    ...
    for epoch in range(epochs):
        ...
        print(f'{model_name} Epoch {epoch}, Loss {avg_loss}')
        if epoch % 10 == 0 or epoch == epochs - 1:
            metrics_all, cov_all, div_all = evaluate(model, dataset, range(1, dataset.n_users + 1), ks, prefix=f'{model_name} All')
            metrics_cold, cov_cold, div_cold = evaluate(model, dataset, dataset.cold_users, ks, prefix=f'{model_name} Cold') if dataset.cold_users else evaluate(model, dataset, [], ks, prefix=f'{model_name} Cold')
            metrics_warm, cov_warm, div_warm = evaluate(model, dataset, dataset.warm_users, ks, prefix=f'{model_name} Warm') if dataset.warm_users else evaluate(model, dataset, [], ks, prefix=f'{model_name} Warm')
            all_metrics.append((metrics_all, cov_all, div_all))
            cold_metrics.append((metrics_cold, cov_cold, div_cold))
            warm_metrics.append((metrics_warm, cov_warm, div_warm))
    # Plot train loss
    ...
    eval_epochs = list(range(0, epochs, 10)) + [epochs - 1] if epochs % 10 != 0 else list(range(0, epochs, 10))
    for k in ks:
        recalls = [m[0][k]['recall'] for m in all_metrics]
        plt.plot(eval_epochs, recalls, label='Recall')
        ndcgs = [m[0][k]['ndcg'] for m in all_metrics]
        plt.plot(eval_epochs, ndcgs, label='NDCG')
        plt.title(f'{model_name} Recall and NDCG @{k}')
        plt.xlabel('Epoch')
        plt.ylabel('Value')
        plt.legend()
        plt.savefig(f'{model_name}_metrics_{k}.png')
        plt.close()
    return all_metrics[-1], cold_metrics[-1], warm_metrics[-1]

if __name__ == '__main__':
    # Baseline: LightGCN
    print("Running LightGCN Baseline")
    ...
    print("Model\tGroup\tK\tPrec\tRecall\tNDCG\tHR\tCoverage\tDiversity")
    for model_name, res in [('LightGCN All', results_baseline[0]), ('DyHuCoG All', results_dyhucog[0])]:
        metrics, cov, div = res
        for k in ks:
            print(f'{model_name}\tAll\t{k}\t{metrics[k]["prec"]:.4f}\t{metrics[k]["recall"]:.4f}\t{metrics[k]["ndcg"]:.4f}\t{metrics[k]["hr"]:.4f}\t{cov:.4f}\t{div:.4f}')
    for model_name, res in [('LightGCN Cold', results_baseline[1]), ('DyHuCoG Cold', results_dyhucog[1])]:
        metrics, cov, div = res
        for k in ks:
            print(f'{model_name}\tCold\t{k}\t{metrics[k]["prec"]:.4f}\t{metrics[k]["recall"]:.4f}\t{metrics[k]["ndcg"]:.4f}\t{metrics[k]["hr"]:.4f}\t{cov:.4f}\t{div:.4f}')
    for model_name, res in [('LightGCN Warm', results_baseline[2]), ('DyHuCoG Warm', results_dyhucog[2])]:
        metrics, cov, div = res
        for k in ks:
            print(f'{model_name}\tWarm\t{k}\t{metrics[k]["prec"]:.4f}\t{metrics[k]["recall"]:.4f}\t{metrics[k]["ndcg"]:.4f}\t{metrics[k]["hr"]:.4f}\t{cov:.4f}\t{div:.4f}')